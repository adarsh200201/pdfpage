...
const TextToPdf = () => {
  ...
  const [conversionSettings, setConversionSettings] = useState({
    ...
    font: "Arial" as "Arial" | "Times" | "Courier",
    fontSize: 12,
  });
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = uploadedFiles.filter((file) => {
      const isValidType = file.name.match(/\.(txt|csv)$/i);
      if (!isValidType) {
        toast({
          title: "Invalid file type",
          description: `${file.name} is not a valid text file. Please upload .txt or .csv files.`,
          variant: "destructive",
        });
        return false;
      }
    ...
    tracking.trackFileUpload("Text", validFiles.length);
  };
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      toast({
        ...
        description: "Please upload text documents to convert to PDF.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          tracking.trackConversionStart("Text", "PDF", [fileStatus.file]);
          ...
          // Use the LibreOffice Text to PDF conversion method
          const result = await PDFService.convertTextToPdfLibreOffice(
            fileStatus.file,
            {
              quality: conversionSettings.quality,
              pageSize: conversionSettings.pageSize,
              orientation: conversionSettings.orientation,
              font: conversionSettings.font,
              fontSize: conversionSettings.fontSize,
            },
          );

          console.log("✅ LibreOffice Text to PDF conversion completed");

          const processingTime = Date.now() - startTime;

          // Create download blob and URL
          const downloadUrl = URL.createObjectURL(result.blob);
          const outputFilename = `${fileStatus.file.name.replace(/\.(txt|csv)$/i, "")}_converted.pdf`;
          ...
          tracking.trackConversionSuccess("Text", "PDF", [fileStatus.file]);
          ...
          tracking.trackConversionError("Text", "PDF", error.message);
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Convert text documents (.txt, .csv) to PDF format using LibreOffice
            engine. Preserve formatting with professional quality output.
          </p>
        </div>
            ...
            <CardContent className="pt-6">
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-gray-400 transition-colors">
                ...
                <p className="text-gray-600 mb-4">
                  Drag and drop your .txt or .csv files here, or click to browse
                </p>
                <input
                  type="file"
                  multiple
            ...
            <Card>
              <CardContent className="pt-6">
                ...
                <p className="text-gray-600">
                  Uses real LibreOffice engine for 100% accurate conversions
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                ...
                <p className="text-gray-600">
                  Maintains text formatting and structure in PDF output
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                ...
                <p className="text-gray-600">
                  Supports .txt and .csv files with customizable options
                </p>
              </CardContent>
            </Card>
          </div>
