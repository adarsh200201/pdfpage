...
const RtfToPdf = () => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = uploadedFiles.filter((file) => {
      const isValidType = file.name.match(/\.rtf$/i);
      if (!isValidType) {
        toast({
          title: "Invalid file type",
          description: `${file.name} is not a valid RTF file. Please upload .rtf files.`,
          variant: "destructive",
        });
        return false;
      }
    ...
    tracking.trackFileUpload("RTF", validFiles.length);
  };
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      toast({
        ...
        description: "Please upload RTF documents to convert to PDF.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          tracking.trackConversionStart("RTF", "PDF", [fileStatus.file]);

          // Use LibreOffice backend service for conversion
          const result = await PDFService.convertRtfToPdfLibreOffice(
            fileStatus.file,
            {
          ...
          const outputFilename = `${fileStatus.file.name.replace(/\.rtf$/i, "")}_converted.pdf`;
          ...
          tracking.trackConversionSuccess("RTF", "PDF", [fileStatus.file]);
          ...
          tracking.trackConversionError("RTF", "PDF", error.message);
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Convert Rich Text Format (.rtf) files to PDF using LibreOffice
            engine. Preserve formatting, images, and layout with professional
            quality.
          </p>
        </div>
            ...
            <Card>
              <CardContent className="pt-6">
                ...
                <p className="text-gray-600">
                  Uses real LibreOffice engine for perfect RTF compatibility
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                ...
                <p className="text-gray-600">
                  Maintains rich text formatting, images, and document structure
                </p>
              </CardContent>
            </Card>
            <Card>
